...
exports.login = function(req, res) {
  ...
  return userDao()
    .findOne({ $or: [{ userName: name }, { email: name }] })
    .then(
      function(user) {
        if (user) {
...
exports.reg = function(req, res) {
  ...
  return userDao()
    .count({ $or: [{ userName: form.userName }, { email: form.email }] })
...
exports.nameValid = function(req, res) {
  ...
  return userDao()
    .count({ $or: [{ userName: name }, { email: name }] })
    .then(
      function(counts) {
        if (counts) {
...
function validateForm(form) {
  ...
  switch (true) {
    ...
    case !form.password:
      msg = '密码不能为空';
      break;
    case form.password !== form.password2:
      msg = '两次密码不一致';
      break;
    case !/^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/.test(form.email):
      msg = '邮箱格式不正确';
      break;
    default:
      break;
