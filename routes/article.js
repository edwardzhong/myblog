...
exports.getArticles=function(req, res){
	var p=JSON.parse(req.query.p||'{}')
		...
		,pageQuery={page:p.page||1,size:p.size||10};
		
	return articleDao.getPage(query,pageQuery).then(function(data){
...
exports.getArticle=function(req,res){
	...
	return articleDao.update({"_id":id},{"$inc":{"viewCounts":1}}).then(function(data){
...
exports.saveArticle=function(req,res){
	...
	return userDao().findOne({userName:name}).then(function(user){
		...
		if(!form._id){
			...
			return articleDao.update({'_id':form._id},{title:form.title,content:form.content,tags:tags,updateDate:new Date()})
			.then(function(data){
				res.json({isOk:true});
			});
		}
...
exports.delArticle=function(req,res){
	var id=req.params.id,
		...
		arr=[];
	return articleDao.findOne({author:userId,_id:id}).then(function(article){
		article.comments.forEach(function(id){//删除相关的评论
...
exports.addComment=function(req,res){
	...
	.spread(function(user,article){
		...
		return commentDao.create({"msg":form.msg,"createDate":new Date(),"author":user._id})
		.then(function(com){
